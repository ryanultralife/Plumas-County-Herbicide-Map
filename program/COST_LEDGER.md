...
| `mill_delivered_log_price` | Mill-delivered log price (pond / gate) — common Sierra sorts | 325–570 (mid 420) | USD_per_MBF | B |
...
| `owner_residual_stumpage_sawlog` | Illustrative residual stumpage (delivered − log − haul) before regen | 195.0–275.0 (mid 250.0) | USD_per_MBF | B |
...
| `owner_net_per_acre` | Net revenue to landowner after logging, haul, regen, release | 780.0–7620.0 | USD_per_acre | C |

**Filled (A/B with numbers): 13 · Gaps (D): 0**

## Next fills
- SPI Quincy delivered price sheet if ever published or shared under NDA for internal model only
- FOIA/PRA unit costs for plant+herbicide on Plumas NF and private THP cost exhibits
- CalTREES bulk export: acres/volume by silvicultural method for Plumas (request pending template)
- GPS truck time samples landing→Quincy scales
- Sawmill recovery factor for accurate mill margin
